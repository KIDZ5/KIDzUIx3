...
You can create a new app by calling the `New` method from the KIDzUIx3 API. This returns a custom object merged with a `ScreenGui`.
...
Use `AppRecorder` when you want a compact, KIDzUIx3-aware dump of the components you create.
...
`recorder:Dump()` returns JSON with a `KIDzUIx3AppDump` root, each recorded component, serializable component properties, and the current root instance properties for each component. Functions are skipped because they cannot be recreated outside Roblox.

If you did not start a recorder, `KIDzUIx3.AppDump(app)` still works, but it falls back to a raw rendered instance tree:
