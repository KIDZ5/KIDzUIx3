...
Before you can begin creating a custom theme or override, you must first get familiar with the KIDzUIx3 `ValueState`.

## What is a `ValueState`?

A `ValueState` is a connectable handle that lets KIDzUIx3 know when a certain value is changed.
...
KIDzUIx3's theme system only support `ValueState`, so you need to get familiar with the concept and reason why `ValueState` is important in a reactive UI system.
